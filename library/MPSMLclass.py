...
class GTN(MPSclass.MPS, MLclass.MachineLearning, Programclass.Program):
    ...
    # Initialize the mps structure with ones method according to the tensor_data.shape, a pixel correspond to a 3D tensor
    # The MPS structure are stored in self.tensor_data, and then the mps are regularized.
    ...
    def update_mps_once(self):
        ...
        tmp_tensor_current = self.tensor_data[tmp_index1] # MPS central tensor
        tmp_tensor1 = torch.einsum(
            'ni,nv,nj->nivj',
            self.environment_left[tmp_index1],
            self.tensor_input[:, tmp_index1, :],
            self.environment_right[tmp_index1]).reshape(self.data_info['n_training'], -1) # 大X张量
        tmp_inner_product = (tmp_tensor1.mm(tmp_tensor_current.view(-1, 1))).t() # 大X和psi的内积
    ...
    # environment Tensor central orthogonal forward
    ...
    # environment Tensor central orthogonal backward
    ...
    def calculate_accuracy(self, data_type='test', force_mode='off'):
        ...
        if data_type in self.accuracy.keys():
            ...
            if self.is_all_gtn_trained or (force_mode == 'on'):
                ...
                for mps_label in self.para['training_label']:
                    tmp_test_inner_product.append(self.inner_product[data_type][str(mps_label)]) # inner_product is fidelity
                tmp_test_inner_product = numpy.abs(numpy.array(tmp_test_inner_product)).argmax(axis=0) # get max fidelity
